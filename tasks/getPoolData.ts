...
export default task("get-pool-data", "Get pools data")
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        if (!taskArgs.name && !taskArgs.creator) {
            const fpd = await hre.ethers.getContract(
                "FusePoolLens",
                (
                    await hre.ethers.getNamedSigner("deployer")
                ).address
            );
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        const maxBorrow = fusePoolData.assets
            .map(
                (a) =>
                    a.supplyBalanceUSD *
                    parseFloat(hre.ethers.utils.formatUnits(a.collateralFactor, a.underlyingDecimals))
            )
            .reduce((a, b) => a + b, 0);
