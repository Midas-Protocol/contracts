...
interface ISynthereumLiquidityPool is ISynthereumLiquidityPoolGeneral {
  ...
  ) external returns (uint256 synthTokensLiquidated, uint256 collateralReceived, uint256 rewardAmount);
   ...
   */
  function feeRecipientsInfo() external view returns (address[] memory, uint32[] memory, uint256);
