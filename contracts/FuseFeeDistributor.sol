 ...
 */
contract FuseFeeDistributor is
    Initializable,
    OwnableUpgradeable,
    ...
    ComptrollerErrorReporter
{
    ...
    function initialize(uint256 _defaultInterestFeeRate) public initializer {
        require(
            _defaultInterestFeeRate <= 1e18,
            "Interest fee rate cannot be more than 100%."
        );
        __Ownable_init();
    ...
    {
        // Make sure comptroller == msg.sender
        (address underlying, address comptroller) = abi.decode(
            constructorData[0:64],
            (address, address)
        );
        require(comptroller == msg.sender, "Comptroller is not sender.");
