...
import { Pyth } from "../pyth/PythOracle.sol";
...
contract PythPriceOracle is IPriceOracle, BasePriceOracle {
   ...
   */
  bytes32 public immutable NATIVE_TOKEN_USD_PRICE_FEED;
   ...
   * @param feeds The Oracle price feed ids for each of `underlyings`.
   */
  function setPriceFeeds(address[] memory underlyings, bytes32[] memory feeds) external onlyAdmin {
    // Input validation
    require(
  ...
  function _price(address underlying) internal view returns (uint256) {
    ...
    bytes32 feed = priceFeeds[underlying];
    ...
    if (address(NATIVE_TOKEN_USD_PRICE_FEED) == address(0)) {
      ...
      int256 tokenUsdPrice = getCurrentPrice(feed).price;
