...
contract UniswapTwapPriceOracleRoot {
    ...
    function price0TWAP(address pair) internal view returns (uint) {
        ...
        return (currPx0Cumu - lastObservation.priceCumulative) / (block.timestamp - lastObservation.timestamp); // overflow is desired
    }
    ...
    function price1TWAP(address pair) internal view returns (uint) {
        ...
        return (currPx1Cumu - lastObservation.priceCumulative) / (block.timestamp - lastObservation.timestamp); // overflow is desired
    }
     ...
     */
    function price(address underlying, address factory) external view returns (uint) {
        // Return ERC20/ETH TWAP
        address pair = IUniswapV2Factory(factory).getPair(underlying, WETH);
        uint256 baseUnit = 10 ** uint256(ERC20Upgradeable(underlying).decimals());
        return (underlying < WETH ? price0TWAP(pair) : price1TWAP(pair)).div(2 ** 56).mul(baseUnit).div(2 ** 56); // Scaled by 1e18, not 2 ** 112
    }
    ...
    function workable(address[] calldata pairs, uint256[] calldata minPeriods, uint256[] calldata deviationThresholds) external view returns (bool[] memory) {
        require(pairs.length > 0 && pairs.length == minPeriods.length && pairs.length == deviationThresholds.length, "Array lengths must be equal and greater than 0.");
        bool[] memory answers = new bool[](pairs.length);
        for (uint256 i = 0; i < pairs.length; i++) answers[i] = _workable(pairs[i], minPeriods[i], deviationThresholds[i]);
        return answers;
    }
    ...
    function _workable(address pair, uint256 minPeriod, uint256 deviationThreshold) internal view returns (bool) {
        // Workable if:
    ...
    function _update(address pair) internal returns (bool) {
        ...
        uint256 priceCumulative;
        if (IUniswapV2Pair(pair).token0() == WETH) priceCumulative = IUniswapV2Pair(pair).price1CumulativeLast();
        else if (IUniswapV2Pair(pair).token1() == WETH) priceCumulative = IUniswapV2Pair(pair).price0CumulativeLast();
        ...
        (, , uint32 lastTime) = IUniswapV2Pair(pair).getReserves();
        observations[pair][observationCount[pair] % OBSERVATION_BUFFER] = Observation(lastTime, priceCumulative);
