 ...
 */
contract MockPriceOracle is PriceOracle, BasePriceOracle {
    ...
    function _price(address underlying) internal view returns (uint) {
        ...
        if (address(ethPriceFeeds[underlying]) != address(0)) {
            ...
            return uint256(tokenEthPrice).mul(1e18).div(10 ** uint256(ethPriceFeeds[underlying].decimals()));

        } else if (address(usdPriceFeeds[underlying]) != address(0)) {
            // mock token/USD = 10 and eth/USD = 4000
            int256 tokenUsdPrice = 10;
            int256 ethUsdPrice = 410011359241;
            return uint256(tokenUsdPrice).mul(1e26).div(10 ** uint256(usdPriceFeeds[underlying].decimals())).div(uint256(ethUsdPrice));
