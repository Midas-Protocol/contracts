...
contract FuseSafeLiquidator is Initializable, IUniswapV2Callee {
    ...
    function exchangeAllEthOrTokens(address from, address to, uint256 minOutputAmount, IUniswapV2Router02 uniswapV2Router) private {
        ...
        if (from == address(0)) {
            if (to == W_NATIVE_ADDRESS) {
                ...
                uniswapV2Router.swapExactETHForTokens{value: address(this).balance}(minOutputAmount, array(W_NATIVE_ADDRESS, to), address(this), block.timestamp);
            }
            ...
            if (to == address(0)) uniswapV2Router.swapExactTokensForETH(inputBalance, minOutputAmount, array(from, W_NATIVE_ADDRESS), address(this), block.timestamp);
            else uniswapV2Router.swapExactTokensForTokens(inputBalance, minOutputAmount, from == W_NATIVE_ADDRESS || to == W_NATIVE_ADDRESS ? array(from, to) : array(from, W_NATIVE_ADDRESS, to), address(this), block.timestamp); // Put W_NATIVE in the middle of the path if not already a part of the path
        }
    }
    ...
    function exchangeToExactEth(address from, uint256 outputAmount, IUniswapV2Router02 uniswapV2Router) private {
        ...
        if (from == W_NATIVE_ADDRESS) {
            ...
            uniswapV2Router.swapTokensForExactETH(outputAmount, inputBalance, array(from, W_NATIVE_ADDRESS), address(this), block.timestamp);
        }
    }
    ...
    function safeLiquidateToTokensWithFlashLoan(address borrower, uint256 repayAmount, ICErc20 cErc20, ICToken cTokenCollateral, uint256 minProfitAmount, address exchangeProfitTo, IUniswapV2Router02 uniswapV2RouterForBorrow, IUniswapV2Router02 uniswapV2RouterForCollateral, IRedemptionStrategy[] memory redemptionStrategies, bytes[] memory strategyData, uint256 ethToCoinbase) external returns (uint256) {
        ...
        bool token0IsUnderlyingBorrow;
        {
            ...
            pair = IUniswapV2Pair(IUniswapV2Factory(uniswapV2RouterForBorrow.factory()).getPair(underlyingBorrow, W_NATIVE_ADDRESS));
...
//        pair.swap(repayAmount, 0, address(0x18c1d6471EE332e9E2141c86807572FB7f6B3921), msg.data);
    ...
    function repayWethFlashLoan(uint256 repayAmount, ICErc20 cErc20Collateral, address exchangeProfitTo, uint256 flashLoanReturnAmount, IUniswapV2Router02 uniswapV2Router, address[] memory redemptionStrategies, bytes[] memory strategyData) private returns (address) {
        ...
        if (address(uniswapV2Router) == UNISWAP_V2_ROUTER_02_ADDRESS && address(underlyingCollateral) == (cErc20Collateral.underlying() == 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48 ? 0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599 : 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48)) {
            ...
            uint256 tokensRequired = getAmountsIn(uniswapV2Router.factory(), repayAmount, array(address(underlyingCollateral), W_NATIVE_ADDRESS))[0];
            require(tokensRequired <= underlyingCollateralSeized, "Flashloan return amount greater than seized collateral.");
            require(underlyingCollateral.transfer(msg.sender, tokensRequired), "Failed to transfer non-W_NATIVE tokens back to flashlender.");
        } else {
            ...
            if (address(underlyingCollateral) != W_NATIVE_ADDRESS) {
                ...
                if (address(underlyingCollateral) == address(0)) {
                    ...
                    if (exchangeProfitTo == address(underlyingCollateral)) uniswapV2Router.swapTokensForExactTokens(flashLoanReturnAmount, underlyingCollateralSeized, array(address(underlyingCollateral), W_NATIVE_ADDRESS), address(this), block.timestamp);
                    else {
                        uniswapV2Router.swapExactTokensForTokens(underlyingCollateralSeized, flashLoanReturnAmount, array(address(underlyingCollateral), W_NATIVE_ADDRESS), address(this), block.timestamp);
            ...
            require(flashLoanReturnAmount <= IERC20Upgradeable(W_NATIVE_ADDRESS).balanceOf(address(this)), "Flashloan return amount greater than W_NATIVE exchanged from seized collateral.");
    ...
    function repayTokenFlashLoan(uint256 repayAmount, ICToken cTokenCollateral, address exchangeProfitTo, uint256 flashLoanReturnAmount, IUniswapV2Router02 uniswapV2RouterForBorrow, IUniswapV2Router02 uniswapV2RouterForCollateral, address[] memory redemptionStrategies, bytes[] memory strategyData, IERC20Upgradeable underlyingBorrow) private returns (address) {
        ...
        if (cTokenCollateral.isCEther()) {
            ...
            uint256 wethRequired = getAmountsIn(uniswapV2RouterForBorrow.factory(), repayAmount, array(W_NATIVE_ADDRESS, address(underlyingBorrow)))[0];
            ...
            require(W_NATIVE.transfer(msg.sender, wethRequired), "Failed to repay Uniswap flashloan with W_NATIVE exchanged from seized collateral.");
            ...
            if (address(underlyingCollateral) == address(underlyingBorrow)) {
                ...
                require(underlyingBorrow.transfer(msg.sender, flashLoanReturnAmount), "Failed to repay token flashloan on borrow (non-W_NATIVE) side.");
                ...
                if (address(underlyingCollateral) != W_NATIVE_ADDRESS) {
                    ...
                    if (exchangeProfitTo == address(underlyingCollateral)) uniswapV2RouterForCollateral.swapTokensForExactTokens(wethRequired, underlyingCollateralSeized, array(address(underlyingCollateral), W_NATIVE_ADDRESS), address(this), block.timestamp);
                    else uniswapV2RouterForCollateral.swapExactTokensForTokens(underlyingCollateralSeized, wethRequired, array(address(underlyingCollateral), W_NATIVE_ADDRESS), address(this), block.timestamp);
                }

                // Repay flashloan
                require(wethRequired <= IERC20Upgradeable(W_NATIVE_ADDRESS).balanceOf(address(this)), "Not enough W_NATIVE exchanged from seized collateral to repay flashloan.");
                require(W_NATIVE.transfer(msg.sender, wethRequired), "Failed to repay Uniswap flashloan with W_NATIVE exchanged from seized collateral.");
