...
contract FuseSafeLiquidator is Initializable, IUniswapV2Callee {
    ...
    function exchangeAllEthOrTokens(
        ...
        if (from == address(0)) {
            ...
            W_NATIVE.withdraw(
                IERC20Upgradeable(W_NATIVE_ADDRESS).balanceOf(address(this))
            );
        } else {
            ...
            if (to == address(0))
                ...
                uniswapV2Router.swapExactTokensForTokens(
                    inputBalance,
                    minOutputAmount,
                    from == W_NATIVE_ADDRESS || to == W_NATIVE_ADDRESS
                        ? array(from, to)
                        : array(from, W_NATIVE_ADDRESS, to),
                    address(this),
                    block.timestamp
            ...
            if (seizedCTokenAmount > 0) {
                ...
                if (cTokenCollateral.isCEther()) {
                    ...
                    IERC20Upgradeable underlyingCollateral = IERC20Upgradeable(
                        ICErc20(address(cTokenCollateral)).underlying()
                    );
        ...
        returns (uint256)
    {
        ...
        if (erc20Contract == address(0)) {
            ...
            require(
                ...
                "Minimum NATIVE output amount not satisfied."
            );
            ...
            if (seizedOutputAmount > 0) {
                ...
                require(
                    success,
                    "Failed to transfer output NATIVE to msg.sender."
                );
            }
        } else {
    ...
    function getAmountsIn(
        ...
        for (uint256 i = path.length - 1; i > 0; i--) {
            ...
            amounts[i - 1] = PancakeLibrary.getAmountIn(
                amounts[i],
                reserveIn,
                reserveOut
            );
    ...
    function repayWethFlashLoan(
        ...
        if (
            ...
            if (address(underlyingCollateral) != W_NATIVE_ADDRESS) {
                ...
                if (address(underlyingCollateral) == address(0)) {
                    ...
                    if (exchangeProfitTo == address(underlyingCollateral))
                        ...
                        underlyingCollateral = IERC20Upgradeable(
                            W_NATIVE_ADDRESS
                        );
                    }
                }
            ...
            require(
                flashLoanReturnAmount <=
                    IERC20Upgradeable(W_NATIVE_ADDRESS).balanceOf(
                        address(this)
                    ),
                "Flashloan return amount greater than W_NATIVE exchanged from seized collateral."
            );
            require(
                W_NATIVE.transfer(msg.sender, flashLoanReturnAmount),
                "Failed to transfer W_NATIVE back to flashlender."
            );
        }
    ...
    function repayTokenFlashLoan(
        ...
        if (cTokenCollateral.isCEther()) {
            ...
            require(
                ...
                "Seized NATIVE collateral not enough to repay flashloan."
            );
