...
contract BoosterGaugesTest is DSTest {
  ...
  function testMarketGauges(uint112 votingPower) public {
    ...
    assertEq(
      aliceRewardsAfter + bobRewardsAfter,
      rewardsForCycle,
      "total rewards claimed should equal the rewards for the cycle"
    );
  }
}
...
contract Booster3070SplitTest is DSTest {
  ...
  function testInterestAccrualFixedParams() public {
    ...
    assertEq(
      aliceRewardsAfter + bobRewardsAfter + roundingErrors,
      rewardsPerSec * rewardsTotalPeriod,
      "total rewards claimed should equal the rewards for the two cycles"
    );
    ...
    assertTrue(
      alicesShareOfRewards == 625185166716251851,
      "alice should get approx 62% of the total rewards (due to the interest accrued)"
    );
  }

  function testFuzzInterestAccrual(
    uint128 supplyAmount,
    uint8 borrowMultiplier,
    uint32 rewardsPerSec
  ) public {
    accrueInterestAndRewards(supplyAmount, borrowMultiplier, rewardsPerSec);
  }

  function accrueInterestAndRewards(
    uint128 supplyAmount,
    uint8 borrowMultiplier,
    uint32 rewardsPerSec
  ) internal {
