...
contract ContractsUpgradesTest is BaseTest {
    ...
    function testFuseFeeDistributorUpgrade() public shouldRun(forChains(BSC_MAINNET)) {
        ...
        (
            address latestCErc20DelegateBefore,
            bool allowResign,
            bytes memory becomeImplementationData
        ) = oldImpl.latestCErc20Delegate(oldCercDelegate);
        ...
        if (whitelistedBefore) emit log("whitelisted before"); else emit log("should be whitelisted");
...
//            emit log_address(admin);
        ...
        (
        address latestCErc20DelegateAfter,
        bool allowResignAfter,
        bytes memory becomeImplementationDataAfter
        ) = ffd.latestCErc20Delegate(oldCercDelegate);
        ...
        if (whitelistedAfter) emit log("whitelisted After"); else emit log("should be whitelisted");
