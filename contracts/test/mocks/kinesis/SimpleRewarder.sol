...
contract SimpleRewarder is IRewarder, BoringOwnable {
  ...
  function init(bytes calldata data) public payable {
    ...
    require(rewardToken != IERC20(address(0)), "Rewarder: bad rewardToken");
    require(IMiniChef(MINICHEF).lpToken(pid) == masterLpToken, "Rewarder: bad pid or masterLpToken");
  ...
  function onSaddleReward(
    ...
    if (user.amount > 0) {
      ...
      rewardToken.transfer(to, pending);
    }
  ...
  function pendingToken(address _user) public view returns (uint256 pending) {
    ...
    if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
      ...
      accToken1PerShare = accToken1PerShare.add(reward.mul(ACC_TOKEN_PRECISION) / lpSupply);
    }
  ...
  function updatePool() public returns (PoolInfo memory pool) {
    ...
    if (block.timestamp > pool.lastRewardTime) {
      ...
      if (lpSupply > 0) {
        uint256 time = block.timestamp.sub(pool.lastRewardTime);
        uint256 reward = time.mul(rewardPerSecond);
        pool.accToken1PerShare = uint128(pool.accToken1PerShare.add(reward.mul(ACC_TOKEN_PRECISION) / lpSupply));
      }
