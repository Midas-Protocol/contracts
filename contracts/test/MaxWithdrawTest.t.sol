...
contract MaxWithdrawTest is WithPool {
  ...
  function testMIIMOMaxWithdraw() public fork(POLYGON_MAINNET) {
    ...
    LiquidationData memory vars;
    vm.roll(1);
    vars.bnb = MockAsset(0xADAC33f543267c4D59a8c299cF804c303BC3e4aC);
    vars.usdc = MockAsset(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174);

    deployCErc20Delegate(address(vars.bnb), "MIMO", "mimo", 0.9e18);
    deployCErc20Delegate(address(vars.usdc), "USDC", "usdc", 0.9e18);
    ...
    // Account One Supply
    {
      emit log("Account One Supply");
      vm.startPrank(accountOne);
      vars.bnb.approve(address(cMimoToken), 1e36);
    ...
    // Account Two Borrow
    {
      ...
      uint256 maxBorrow = poolLensSecondary.getMaxBorrow(accountTwo, ICToken(address(cUSDC)));
    ...
    // Account One Borrow
    {
      ...
      vars.usdc.approve(address(cUSDC), 1e36);
      cUSDC.borrow(0.5e6);
      assertEq(cUSDC.totalBorrows(), 0.5e6, "!cUSDC total borrows");

      uint256 maxWithdraw = poolLensSecondary.getMaxRedeem(accountOne, ICToken(address(cMimoToken)));

      uint256 beforeMimoBalance = vars.bnb.balanceOf(accountOne);
      cMimoToken.redeemUnderlying(type(uint256).max);
      uint256 afterMimoBalance = vars.bnb.balanceOf(accountOne);
