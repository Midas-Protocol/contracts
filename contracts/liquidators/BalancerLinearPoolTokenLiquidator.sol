...
contract BalancerLinearPoolTokenLiquidator is IRedemptionStrategy {
  function redeem(
    IERC20Upgradeable inputToken,
    uint256 inputAmount,
    bytes memory strategyData
  ) external override returns (IERC20Upgradeable outputToken, uint256 outputAmount) {
    (address poolAddress, address outputTokenAddress) = abi.decode(strategyData, (address, address));

    IBalancerPool pool = IBalancerPool(address(poolAddress));
  ...
  function name() public pure returns (string memory) {
    return "BalancerLinearPoolTokenLiquidator";
  }
}
