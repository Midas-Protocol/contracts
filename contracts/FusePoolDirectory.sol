    ...
    ComptrollerErrorReporter
{
    ...
    function deployPool(
        ...
        bytes memory unitrollerCreationCode = abi.encodePacked(
            type(Unitroller).creationCode,
            constructorData
        );
        ...
        require(
            comptrollerProxy._setPriceOracle(IPriceOracle(priceOracle)) == 0,
        ...
        returns (uint256[] memory, FusePool[] memory)
    {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            try IComptroller(pools[i].comptroller).enforceWhitelist() returns (
                bool enforceWhitelist
            ) {
