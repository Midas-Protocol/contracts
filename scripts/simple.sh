...
npx hardhat market:create --asset-config "$POOL_NAME,deployer,CErc20PluginDelegate,$TOUCH,$IRM,0.01,60,10,0,true,$STRATEGY,," --network localhost
...
npx hardhat pools:deposit --amount 150 --symbol TOUCH --pool-address $POOL_ADDRESS --enable-collateral true --account bob --network localhost
npx hardhat pools:deposit --amount 150 --symbol TRIBE --pool-address $POOL_ADDRESS --enable-collateral true --account alice --network localhost
npx hardhat pools:borrow --amount 1 --symbol TRIBE --pool-address $POOL_ADDRESS --account bob --network localhost
