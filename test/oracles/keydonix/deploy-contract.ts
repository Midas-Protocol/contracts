...
import { UniswapV2Factory, UniswapV2Pair, TestErc20, /*PriceEmitter,*/ BlockVerifierWrapper, MerklePatriciaVerifierWrapper, Event } from './generated/uniswap';
...
export async function deployAllTheThings(rpc: SignerFetchRpc) {
	...
	const uniswapFactory: UniswapV2Factory = new UniswapV2Factory(dependencies, uniswapFactoryAddress)
	async function getOrCreatePair() {
		...
		const events: Array<Event> = await uniswapFactory.createPair(appleTokenAddress, bananaTokenAddress)
		console.log(`events : find pair created event ${JSON.stringify(events)}`)
